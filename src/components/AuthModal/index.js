...
import {
  Grid, Button, FormControl, IconButton, InputLabel, OutlinedInput, TextField,
} from '@mui/material';
...
export function AuthModal() {
  const {
    setCurrentUser, setModalState, isTabSignUp, setIsTabSignUp, setSnackBarState,
  ...
  const validateEmail = (email) => {
    ...
    if (isTabSignUp && email && !regex.test(email)) {
  ...
  const signUp = () => {
    if (isFormValid()) {
      ...
      api.signUp({
        name, about, avatar, email, emailPasswordSaltHash,
      })
        .then((res) => {
          if ('error' in res) {
    ...
    <Modal open={authModal.isOpen} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description">
      <Box sx={style}>
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={12}>
            <TextField fullWidth label="Email" variant="outlined" error={isEmailInvalid} helperText={emailHelperText} required value={email} onChange={({ target }) => setEmail(target.value)} onBlur={({ target }) => validateEmail(target.value)} />
          </Grid>
          <Grid item xs={12}>
            <TextField
              fullWidth
